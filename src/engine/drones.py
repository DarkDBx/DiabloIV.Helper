...
def launch_drones(drone_num):
    ...
    if drone_num != 0:
        ...
        input_helper.press(('shift', 'l'))

        for tries in range(1, 25):
            drones_launched = controller.mlocate(
                './assets/indicators/drones/0_drone_in_bay.bmp')
...
def recall_drones(drone_num):
    ...
    if drone_num != 0:
        ...
        for tries in range(1, 30):
            drones_returned = controller.mlocate('./assets/indicators/drones/' +
                                         (str(drone_num)) +
