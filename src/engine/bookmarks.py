...
from engine import navigation as nav, controller
from engine.navigation import wait_for_dock
...
def set_dest():
    ...
    isn't blacklisted."""
    target_dest = 1
    dest = controller.mlocate('./assets/dest/dest' + (str(target_dest)) + '.bmp',
                      conf=conf, loctype='c')
    ...
    if dest != 0:
        ...
        (x, y) = dest
        input_helper.mo._move_to((x + (random.randint(-1, 200))),
                   (y + (random.randint(-3, 3))))
        input_helper.rightClick()
        input_helper.mo._move_to((0 + (random.randint(10, 80))),
...
def is_home():
    ...
    if controller.mlocate('./assets/dest/at_dest0.bmp') == 0:
        logging.debug('not at home station')
        return 0
    elif controller.mlocate('./assets/dest/at_dest0.bmp') != 0:
...
def set_home():
    ...
    home = controller.mlocate('./assets/dest/dest0.bmp', loctype='c')
    if home != 0:
...
def warp_to_local_bookmark(target_site_num):
    ...
    target_site_bookmark = controller.mlocate('./assets/dest/at_dest' + (str(
        target_site_num)) + '.bmp', conf=0.8, loctype='c')

    if target_site_bookmark != 0:
        (x, y) = target_site_bookmark
        input_helper.mo._move_to((x + (random.randint(10, 200))),
                   (y + (random.randint(-3, 3))))
        input_helper.rightClick()
        approach_location = controller.mlocate(
            './assets/buttons/detect_warp_to_bookmark.bmp', conf=conf)
        ...
        if approach_location != 0:
            ...
            input_helper.press('esc')  # Close right-click menu.
            return 0

        # If the 'approach location' option is not found, look for a 'warp
        # to' option and select it.
        elif approach_location == 0:
            warp_to_site = controller.mlocate('./assets/buttons/warp_to_bookmark.bmp',
                                      conf=conf, loctype='c')
...
def dock_at_local_bookmark():
    ...
    dock = controller.mlocate('./assets/dest/at_dest0.bmp', loctype='c')
    if dock != 0:
        (x, y) = dock
        input_helper.mo._move_to((x + (random.randint(-1, 200))),
                   (y + (random.randint(-3, 3))))
...
def detect_bookmark_location():
    ...
    at_dest = controller.mlocate('./assets/dest/at_dest' + (str(n)) + '.bmp', conf=0.8,
                         loctype='c')

    while at_dest == 0:
        n += 1
        logging.debug('looking if at destination ' + (str(n)))
        at_dest = controller.mlocate('./assets/dest/at_dest' + (str(n)) + '.bmp',
                             conf=0.8, loctype='c')
...
def blacklist_station():
    ...
    if at_dest != 0:
        ...
        at_dest = pag.locateCenterOnScreen(
            ('./assets/dest/at_dest' + (str(n)) + '.bmp'),
            confidence=conf,
            region=(originx, originy, windowx, windowy))

        (at_destx), (at_desty) = at_dest
        input_helper.mo._move_to((at_destx + (random.randint(-1, 200))),
...
def blacklist_local_bookmark():
    ...
    bookmark_to_blacklist = pag.locateCenterOnScreen(
        ('./assets/dest/at_dest' + (str(bookmark)) + '.bmp'),
        confidence=conf,
        region=(originx, originy, windowx, windowy))
    ...
    while bookmark_to_blacklist != 0:
        ...
        if bookmark_to_blacklist != 0:
            ...
            input_helper.mo._move_to((bookmark_to_blacklistx + (random.randint(-1, 200))),
            ...
            at_bookmark = pag.locateCenterOnScreen(
                './assets/buttons/detect_warp_to_bookmark.bmp',
                confidence=conf,
                region=(originx, originy, windowx, windowy))
...
def blacklist_set_bookmark(target_site):
    ...
    bookmark_to_blacklist = pag.locateCenterOnScreen(
        ('./assets/dest/at_dest' + (str(target_site)) + '.bmp'),
        confidence=conf,
        region=(originx, originy, windowx, windowy))
    ...
    input_helper.leftClick()
    time.sleep(float(random.randint(3000, 4000)) / 1000)
    input_helper.press('home')
    input_helper.press('b')
    time.sleep(float(random.randint(0, 1000)) / 1000)
