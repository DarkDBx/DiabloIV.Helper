...
from engine.controller import originx, originy, windowx, windowy, conf
...
def is_jammed(detect_jam, haystack=0):
    ...
    if detect_jam == 1 and haystack == 0:
        ...
        if pag.locateOnScreen('./assets/overview/jammed_overview.bmp',
        ...
        if controller.mlocate('./assets/overview/jammed_overview.bmp', haystack=haystack,
                      grayscale=True) != 0:
            logging.info('ship has been jammed!')
            return 1
        else:
...
def build_ship_list(detect_npcs_var, npc_frig_dest, npc_cruiser_bc,
    ...
    if detect_npcs_var == 1:
        if npc_frig_dest == 1:
            npc_list.append(
                './assets/overview/npc_ship_icons/npc_hostile_frigate.bmp')
        if npc_cruiser_bc == 1:
            npc_list.append(
                './assets/overview/npc_ship_icons/npc_hostile_cruiser.bmp')
    pc_list = []
    if detect_pcs_var == 1:
        ...
        if pc_indy == 1:
            pc_list.append(
                './assets/overview/player_ship_icons/archetype_icons'
        ...
        if pc_frig_dest == 1:
            pc_list.append(
                './assets/overview/player_ship_icons'
    ...
    if haystack == 0:
        ...
        if len(npc_list) != 0 or len(pc_list) != 0:
            overview = pag.screenshot(
                region=((originx + (windowx - (int(windowx / 3)))),
                        originy, (int(windowx / 3)), windowy))

            # Only search for icons if the list has at least 1 item
            if len(npc_list) != 0:
                for npc in npc_list:
                    npc_found = controller.mlocate(npc, haystack=overview, conf=conf)
                    if npc_found != 0:
                        logging.debug(
            ...
            if len(pc_list) != 0:
                for pc in pc_list:
                    pc_found = controller.mlocate(pc, haystack=overview, conf=conf)
                    if pc_found != 0:
                        logging.debug(
        ...
        if len(npc_list) != 0:
            for npc in npc_list:
                npc_found = controller.mlocate(npc, haystack=haystack, conf=conf)
                if npc_found != 0:
                    logging.debug(
                        'found ' + (str(npc)) + ' at ' + (str(npc_found)))
        ...
        if len(pc_list) != 0:
            for pc in pc_list:
                pc_found = controller.mlocate(pc, haystack=haystack, conf=conf)
                if pc_found != 0:
                    logging.debug(
                        'found ' + (str(pc)) + ' at ' + (str(pc_found)))
...
def focus_client():
    ...
    input_helper.mo._move_to((originx + (random.randint(50, 300))), (originy + (random.randint(300, 500))))
...
def focus_overview():
    ...
    input_helper.mo._move_to((x + randx), (y + randy))
...
def select_overview_tab(tab):
    ...
    selected = controller.mlocate('./assets/overview/' + (str(tab)) +
                          '_overview_tab_selected.bmp', conf=conf,
                          grayscale=True)
    ...
    if selected != 0:
        ...
        unselected = controller.mlocate('./assets/overview/' + (str(tab)) +
                                '_overview_tab.bmp', loctype='co',
                                grayscale=True)

        if unselected != 0:
            (x, y) = unselected
            input_helper.mo._move_to((x + (random.randint(-12, 12))),
...
def look_for_targets(target1, target2, target3, target4, target5):
    ...
    for t in target_list:
        target = controller.mlocate(t, haystack=overview, conf=conf)
        if target is not None:
            logging.debug('found ' + (str(t)) + ' at ' + (str(target)))
            # (x, y, l, w) = target
            # Move mouse over target for debugging.
            # pag.moveTo((x + (originx + (windowx - (int(windowx / 3))))),
            #           (y + originy),
            #           1, mouse.path())
            return target
        elif target == 0:
...
def is_target_lockable():
    ...
    if controller.mlocate('./assets/indicators/target_lock_available.bmp',
                  conf=conf) != 0:
        logging.debug('within targeting range')
        return 1
    elif controller.mlocate('./assets/indicators/target_lock_available.bmp',
                    conf=conf) == 0:
...
def wait_for_target_lock():
    ...
    for tries in range(1, 50):
        target_locked = controller.mlocate('./assets/indicators/target_lock_attained.bmp')
...
def initiate_target_lock(target):
    ...
    if target != 0:
        ...
        x = (x + (originx + (windowx - (int(windowx / 3)))))
        y = (y + originy)
        input_helper.mo._move_to((x + (random.randint(-100, 20))),
        ...
        for tries in range(1, 6):
            ...
            for approachtime in range(1, 50):
                ...
                if is_target_lockable() == 1 and is_jammed(1) == 0:
                    ...
                    lock_icon = controller.mlocate(
                        './assets/indicators/target_lock_available.bmp',
                        loctype='c', grayscale=True)
                    (x, y) = lock_icon
                    input_helper.mo._move_to((x + (random.randint(-10, 10))),
